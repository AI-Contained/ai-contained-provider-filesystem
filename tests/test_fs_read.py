...
def describe_fs_read():
    ...
    def describe_line_mode():
        async def it_reads_entire_file(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read", {"operations": [{"mode": "Line", "path": expected["subject"]}]}
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading file: {expected['subject']}, all lines (using tool: read)"
                )
        ...
        async def it_reads_a_line_range(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Line", "path": expected["subject"], "start_line": 2, "end_line": 4}]},
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading file: {expected['subject']}, from line 2 to 4 (using tool: read)"
                )
        ...
        async def it_reads_using_negative_line_index(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Line", "path": expected["subject"], "start_line": -2, "end_line": -1}]},
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading file: {expected['subject']}, from line 4 to end of file (using tool: read)"
                )
                assert_that(result.content[0].text).is_equal_to("bravo\ndelta")
        ...
        async def it_reads_single_line(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Line", "path": expected["subject"], "start_line": 1, "end_line": 1}]},
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading file: {expected['subject']}, from line 1 to 1 (using tool: read)"
                )
                assert_that(result.content[0].text).is_equal_to("alpha")
        ...
        async def it_clamps_end_line_beyond_eof(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Line", "path": expected["subject"], "start_line": 1, "end_line": 999}]},
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading file: {expected['subject']}, from line 1 to 999 (using tool: read)"
                )
                assert_that(result.content[0].text).is_equal_to("alpha\nbravo\ncharlie\nbravo\ndelta\n")
        ...
        async def it_returns_end_line_when_start_greater_than_end(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Line", "path": expected["subject"], "start_line": 4, "end_line": 2}]},
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading file: {expected['subject']}, from line 4 to 2 (using tool: read)"
                )
                assert_that(result.content[0].text).is_equal_to("bravo")
        ...
        async def it_errors_on_start_line_beyond_eof(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Line", "path": expected["subject"], "start_line": 999}]},
                    raise_on_error=False,
                )
                assert_that(result.is_error).is_true()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading file: {expected['subject']}, from line 999 to end of file (using tool: read)"
                )
                assert_that(result.content[0].text).is_equal_to(
                    "starting index: 999 is outside of the allowed range: (-5, 5)"
                )
        ...
        async def it_errors_on_line_zero(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Line", "path": expected["subject"], "start_line": 0}]},
                    raise_on_error=False,
                )
                assert_that(result.is_error).is_true()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading file: {expected['subject']}, from line 6 to end of file (using tool: read)"
                )
                assert_that(result.content[0].text).is_equal_to(
                    "starting index: 0 is outside of the allowed range: (-5, 5)"
                )
        ...
        async def it_errors_on_nonexistent_file(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Line", "path": "/code/volatile/ghost.txt"}]},
                    raise_on_error=False,
                )
                assert_that(result.is_error).is_true()
                assert_that(h.messages).is_empty()
                assert_that(result.content[0].text).is_equal_to(
                    "Failed to validate tool parameters: '/code/volatile/ghost.txt' does not exist"
                )
    ...
    def describe_search_mode():
        async def it_finds_matching_lines(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read", {"operations": [{"mode": "Search", "path": expected["subject"], "pattern": "bravo"}]}
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Searching: {expected['subject']} for pattern: bravo (using tool: read)"
                )
                assert_that(result.content[0].text).is_equal_to(
        ...
        async def it_returns_empty_list_when_no_matches(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Search", "path": expected["subject"], "pattern": "nonexistent"}]},
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Searching: {expected['subject']} for pattern: nonexistent (using tool: read)"
                )
                assert_that(result.content[0].text).is_equal_to("[]")
        ...
        async def it_matches_literally_not_as_regex(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read", {"operations": [{"mode": "Search", "path": expected["subject"], "pattern": "bra.o"}]}
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Searching: {expected['subject']} for pattern: bra.o (using tool: read)"
                )
                assert_that(result.content[0].text).is_equal_to("[]")
        ...
        async def it_respects_context_lines_zero(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    ...
                    {
                        "operations": [
                            {"context_lines": 0, "mode": "Search", "path": expected["subject"], "pattern": "bravo"}
                        ]
                    },
                )
                assert_that(result.is_error).is_false()
    ...
    def describe_directory_mode():
        async def it_lists_directory_entries(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read", {"operations": [{"mode": "Directory", "path": str(dirtest)}]}
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading directory: {dirtest} (using tool: read, max depth: 0, max entries: 1000, excluding: defaults)"
                )
                assert_that(result.content[0].text).matches(
                    r"# Total entries: 2\n\n" + _ls_entry(str(dirtest / "b.txt")) + _ls_entry(str(dirtest / "a.txt"))
                )
        ...
        async def it_lists_recursively_with_depth(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read", {"operations": [{"depth": 1, "mode": "Directory", "path": str(dirtest)}]}
                )
                assert_that(result.is_error).is_false()
                assert_that(h.messages[0]).is_equal_to(
                    f"Reading directory: {dirtest} (using tool: read, max depth: 1, max entries: 1000, excluding: defaults)"
                )
                assert_that(result.content[0].text).matches(
                    r"# Total entries: 2\n\n" + _ls_entry(str(dirtest / "b.txt")) + _ls_entry(str(dirtest / "a.txt"))
                )
        ...
        async def it_errors_on_nonexistent_directory(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read",
                    {"operations": [{"mode": "Line", "path": "/code/volatile/no_such_dir/file.txt"}]},
                    raise_on_error=False,
                )
                assert_that(result.is_error).is_true()
                assert_that(h.messages).is_empty()
                assert_that(result.content[0].text).is_equal_to(
                    "Failed to validate tool parameters: '/code/volatile/no_such_dir/file.txt' does not exist"
                )
    ...
    def describe_decline():
        async def it_returns_cancelled_when_user_declines(mcp, expected):
            ...
            async with Client(transport=mcp, elicitation_handler=h) as client:
                result = await client.call_tool(
                    "fs_read", {"operations": [{"mode": "Line", "path": expected["subject"]}]}, raise_on_error=False
                )
                assert_that(result.is_error).is_true()
